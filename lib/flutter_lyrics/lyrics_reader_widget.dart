...
import 'package:shabadguru/flutter_lyrics/lyrics_reader_paint.dart';
...
class LyricsReader extends StatefulWidget {
  ...
  final double fontSizeOfMain;
  final double fontSizeOfExit;
  final double fontSizeOfMid;

  final double fontSizeOfSpanish;
  final double fontSizeOfHindi;
  ...
  LyricsReader(
      ...
      this.playing,
      this.emptyBuilder,
      required this.fontSizeOfMain,
      required this.fontSizeOfExit,
      required this.fontSizeOfMid,
      required this.fontSizeOfSpanish,
      required this.fontSizeOfHindi})
...
class LyricReaderState extends State<LyricsReader>
  ...
  void initState() {
    super.initState();
    lyricPaint = LyricsReaderPaint(widget.model, widget.ui,widget.isDarkMode)
      ...
      var drawInfo = LyricDrawInfo()
        ..playingExtTextPainter = getTextPaint(
            ...
            widget.ui.getPlayingExtTextStyle().copyWith(
                  color: const Color(0XFFA66F23),
                  fontSize: widget.fontSizeOfExit,
                ),
            size: size)
        ...
        ..playingMainTextPainter = getTextPaint(
            ...
            widget.ui.getPlayingMainTextStyle().copyWith(
                  ...
                  fontSize: widget.fontSizeOfMain,
                ),
            size: size)
        ..otherMainTextPainter = getTextPaint(element.mainText,
            widget.ui.getOtherMainTextStyle().copyWith(color: Colors.grey),
            size: size)
        ..playingMidTextPainter = getTextPaint(
            element.midText,
            widget.ui.getPlayingExtTextStyle().copyWith(
                  color: const Color(0XFFCD774E),
                  fontSize: widget.fontSizeOfMid,
                ),
            size: size)
        ..otherMidTextPainter = getTextPaint(element.midText,
            widget.ui.getOtherExtTextStyle().copyWith(color: Colors.grey),
            size: size)
        ..playingSpanishTextPainter = getTextPaint(
            element.spanishText,
            widget.ui.getPlayingSpanishTextStyle().copyWith(
                  color: const Color(0XFFCD774E),
                  fontSize: widget.fontSizeOfSpanish,
                ),
            size: size)
        ..otherSpanishTextPainter = getTextPaint(element.spanishText,
            widget.ui.getOtherSpanishTextStyle().copyWith(color: Colors.grey),
            size: size)
        ..playingHindiTextPainter = getTextPaint(
            element.hindiText,
            widget.ui.getPlayingHindiTextStyle().copyWith(
                  color: const Color(0XFFCD774E),
                  fontSize: widget.fontSizeOfHindi,
                ),
            size: size)
        ..otherHindiTextPainter = getTextPaint(element.hindiText,
            widget.ui.getOtherHindiTextStyle().copyWith(color: Colors.grey),
            size: size);
  ...
  Container buildReaderWidget() {
    return Container(
      ...
      child: GetBuilder<MusicPlayerController>(builder: (controller) {
